...
export function ControlPanel({
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          alert(
            "Error reading template file. Please ensure it's a valid JSON file."
          );
        }
      };
  ...
  const handleExportTemplate = () => {
    if (currentTemplate) {
      ...
      downloadFile(
        data,
        `meeting-template-${formatDate(new Date())}.json`,
        "application/json"
      );
    }
  };
  ...
  const handleExportNotes = () => {
    if (currentTemplate) {
      ...
      notesContent += `Date: ${new Date(
        currentTemplate.meetingDate
      ).toLocaleDateString()}\n`;
      ...
      downloadFile(
        notesContent,
        `meeting-notes-${formatDate(new Date())}.txt`,
        "text/plain"
      );
    }
  };
  ...
  const handleLoadTemplate = (
    templateType: keyof typeof finalExpenseTemplates
  ) => {
