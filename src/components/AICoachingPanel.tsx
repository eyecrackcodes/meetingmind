...
import { Bot, Brain, Lightbulb, MessageSquare, CheckCircle, AlertTriangle } from 'lucide-react'
...
Make your advice specific to final expense operations and this meeting type.`
      ...
      setLastError(error instanceof Error ? error.message : 'Failed to get coaching advice')
    } finally {
...
Please provide a specific, practical answer that I can use in this meeting context.`
      ...
      setLastError(error instanceof Error ? error.message : 'Failed to get answer')
    } finally {
      setIsAnalyzing(false)
    }
  }
  ...
  const progressStatus = () => {
    ...
    if (percentage < 25) return { icon: AlertTriangle, color: 'text-orange-600', message: 'Just getting started' }
    if (percentage < 50) return { icon: MessageSquare, color: 'text-blue-600', message: 'Making good progress' }
    if (percentage < 75) return { icon: Lightbulb, color: 'text-purple-600', message: 'Excellent momentum' }
    return { icon: CheckCircle, color: 'text-green-600', message: 'Nearly complete!' }
  }
      ...
      <CardContent className="space-y-4">
        ...
        {coachingAdvice && (
          <div className="bg-white border border-green-200 p-4 rounded-lg">
            <div className="flex items-start gap-2 mb-2">
              ...
              <h4 className="font-semibold text-green-800">AI Coaching Advice</h4>
            </div>
        ...
        <div className="bg-gradient-to-r from-green-50 to-blue-50 p-3 rounded-lg border border-green-200">
          <p className="text-xs text-green-700">
            <strong>Pro Tip:</strong> Use the AI coach throughout your meeting for real-time guidance on 
            critical thinking application, content coverage, and final expense best practices.
          </p>
        </div>
      </CardContent>
    </Card>
